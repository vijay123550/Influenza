...
%%
epsilon_1 = linspace(0.4,0.9,51) ;
New_Cov_11 = linspace(40,90,51) ;
[X,Y] = meshgrid(New_Cov_11,epsilon_1);

%%
for i = 1: length(epsilon_1) 
    for j = 1: length(New_Cov_11) 
epsilon1 = epsilon_1(i);
...
hopi(i,j) = sum(y0(:,16))+sum(y0(:,15))+sum(y0(:,14))+sum(y0(:,13));
...
figure(1)
     pcolor(New_Cov_11,epsilon_1,all_group);
 hold on
 %shading interp
 colorbar
val = 4.8040;         % Value of Z to plot contour for
    tol = .004;         % numerical tolerance
 ...
 set(gca,'XTick',[40:10:90],'yticklabels',{'40', '50','60','70','80','90'},'fontsize',16)
   ...
   xlabel('Vaccination Coverage in G1 (in %)')
    ...
    con110 = log10(death);
figure(3)
pcolor(New_Cov_11,epsilon_1,con110 )
 hold on
val = 3.0759;         % Value of Z to plot contour for
    tol = .001;         % numerical tolerance
