...
%%% Annual per %
%all_group(i,j) = (kappa.*q.*(y0(:,5).*1.57 + y0(:,6).*0.965 + y0(:,7)*1.325 + y0(:,8)./3.41))/51748738*100;
%%%%% Total deaths
death(i,j) = sum(d(1) * y0(:,13) + d(2) * y0(:,14) + d(3) * y0(:,15) + d(4) * y0(:,16)./1.4) ;
hopi(i,j) = sum(y0(:,16)./1.4)+sum(y0(:,15))+sum(y0(:,14))+sum(y0(:,13));
...
pcolor(New_Cov_41,epsilon_4,all_group)
 ...
 %shading interp
 colorbar
val = 7.63;         % Value of Z to plot contour for
...
pcolor(New_Cov_41,epsilon_4,con110)
 ...
 %shading interp
 colorbar
val1 = 3.3231;         % Value of Z to plot contour for
